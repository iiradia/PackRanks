...
def save_course_data(catalog_data, num_to_show, term):
    """
    ...
    for rec in catalog_data:
        ...
        #save relevant data for first section
            if '2021' in term:
                try:
                    prof_data = grades_db.spring2021new.find_one(
            ...
            try:
                if (int(prof_data["seats_open"]) > 0 or prof_data["seats_status"] == "Waitlisted" or prof_data["seats_status"] == "Reserved"):
...
def gepRoute():
    ...
    if '2021' not in term_requested:
        ...
        #return json.dumps({"success":"no"}), 400

        #access  collection with the correct data
        catalog_data = grades_db.spring2021new.aggregate([
            {
                "$match" : {
                    "gep" :{"$regex": geps_regex},
                    "course_type": {"$regex": course_type_s},
    ...
    relevant_data = save_course_data(catalog_data, 15, term_requested)
    return relevant_data
...
def deptRoute():
    ...
    if (level_min!="ANY" and level_max!="ANY"):
        ...
        invalid_bounds = [i for i in level_l if i<100 or i>899 and i!="ANY"]
    ...
    if level_min == level_max:
        course_types = ["Lecture", "Problem Session", "Lab"]
    elif "HES" in dept_requested:
    ...
    # should not show problem sessions
        ...
        course_types = ["Lecture"]
    elif 'E' in dept_requested or 'PY' in dept_requested:
        course_types = ["Lecture", "Problem Session", "Lab"]
    else:
    ...
    if '2021' not in term_requested:
        ...
        """
        For now, removed "professor": { "$ne": "Staff"}
        from query since many professors are Staff.
        Will add back later if necessary.
        """
        #access  collection with the correct data
        catalog_data = grades_db.spring2021new.aggregate([
            {
    ...
    relevant_data = save_course_data(catalog_data, 100, term_requested)
